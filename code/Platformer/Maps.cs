  ...
  public static class Level {
    ...
    public static void Setup(string mapPath) {
      ...
      void SetupInteractableObject(TmxObject obj, ref List<Groups> assignedGroups, ZLayers layer, Vector2 position, Texture2D image, Directions faceDirection, string name, bool active, Fragment trim) {
        ...
        if (image == null) {
          string gidInfo = obj.Gid.HasValue ? $" (Gid: {obj.Gid.Value})" : " (No Gid)";
          Console.WriteLine($"[WARNING]: No texture found for object '{name}'{gidInfo} at position ({position.X}, {position.Y}). Skipping sprite creation.");
          return;
        }
      ...
      void SetupCollectable(TmxObject obj, ref List<Groups> assignedGroups, ZLayers layer, Vector2 position, Texture2D image, Directions faceDirection, string name, bool active, ref byte spawnOrbIndex) {
        ...
        if (image == null) {
          string gidInfo = obj.Gid.HasValue ? $" (Gid: {obj.Gid.Value})" : " (No Gid)";
          Console.WriteLine($"[WARNING]: No texture found for collectable '{name}'{gidInfo} at position ({position.X}, {position.Y}). Skipping sprite creation.");
          return;
        }
    ...
    private static void InteractableCollision() {
      ...
      if (CollisionResult != null) {
        if (CollisionResult is TrimmedSprite TrimmedOrb) {
          GoalFragments += (byte)(IsHalf(TrimmedOrb.FragmentType) ? Gameplay.GoalFragmentMax >> 1 : Gameplay.GoalFragmentMax >> 2);
          TrimmedOrb.Destroy();

          if (GoalFragments == Gameplay.GoalFragmentMax) {
      ...
      if (CollisionResult != null && CollisionResult is Spring Spring)
        Spring.Activate(CollisionDirection(Globals.Player, Spring, Directions.All), Globals.Player);
