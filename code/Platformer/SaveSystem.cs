  ...
  public static class SaveSystem {
    ...
    public static void SaveSettings() {
      try {
        ...
        Set(ref SettingsStats, 1 << 1, SoundManager.Muted);
    ...
    public static void Save() {
      try {
        LevelStats Current = PlatformerLevel.Stats;

        if (CurrentLevel >= LevelSaveCount)
          LevelSaveCount = (byte)(CurrentLevel + 1);
    ...
    public static Task LoadSettings() {
      ...
      return Task.Run(() => {
        try {
          ...
          Set(ref PauseMenu.Stats, (ushort)MenuFlags.Fullscreen, IsSet(SettingsStats, 1 << 0));
          Set(ref PauseMenu.Stats, (ushort)MenuFlags.Mute, IsSet(SettingsStats, 1 << 1));
          ...
          for (int Index = 0; Index < PauseMenu.SelectedValues.Length; Index++) {
            ...
            if (Index == 0) { 
              Set(ref PauseMenu.Stats, (ushort)MenuFlags.UpdateScreen, true);
              Set(ref PauseMenu.Stats, (ushort)MenuFlags.UpdateBrightness, true);
              Set(ref PauseMenu.Stats, (ushort)MenuFlags.UpdateFPS, true);
            } else if (Index == 1) {
              Set(ref PauseMenu.Stats, (ushort)MenuFlags.UpdateVolume, true);
              Set(ref PauseMenu.Stats, (ushort)MenuFlags.UpdateSFX, true);
              Set(ref PauseMenu.Stats, (ushort)MenuFlags.UpdateMusic, true);
            }
          }
        } catch (Exception Exception) {
